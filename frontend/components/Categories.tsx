...
const Categories = () => {
  ...
  return (
    <div className="pt-10 px-32  bg-gray-100">
      <div className="flex flex-col relative bg-white rounded-[32px] ">
        <div className="flex text-[#0e162e] px-11 py-5 text-center font-['Inter-Bold',_sans-serif] text-2xl font-bold">
          TOP CATEGORIES
        </div>
        <div className=" grid grid-rows-2 grid-cols-9">
          {paginatedItems.map((item) => (
            <CategoryItem title={item.title} key={item.id} href={item.href}>
              <Image src={item.src} width={144} height={143} alt={item.title} />
            </CategoryItem>
          ))}
        </div>
