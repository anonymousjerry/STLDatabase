...
const DashboardProductDetails = ({
  ...
  const deleteProduct = async () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/products/${id}`, requestOptions)
      .then((response) => {
  ...
  const uploadFile = async (file: any) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/main-image`, {
        method: "POST",
        body: formData,
      });
  ...
  const fetchProductData = async () => {
    ...
    const imagesData = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/images/${id}`, {
      cache: "no-store",
    });
